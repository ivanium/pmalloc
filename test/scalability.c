...
static uint64_t iteration_count = 20000;
static unsigned int thread_count = 2;
...
void *run_test(void *arg) {
  ...
  pthread_barrier_wait(&barrier);
  unsigned int pt = tid;
  executionTime[pt % thread_count] =
      adjusted.tv_sec + adjusted.tv_usec / 1000000.0;
  //  printf ("Thread %u adjusted timing: %d.%06d seconds for %d requests" " of
  //  %d bytes.\n", pt, adjusted.tv_sec, adjusted.tv_usec, total_iterations,
  //  request_size);

  return NULL;
}

void *dummy(unsigned i) { return NULL; }
